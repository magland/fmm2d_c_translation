c     test_hndiv2d.f - differential test for the C translation of hndiv2d.
c
c     Calls both hndiv2d (Fortran reference, from libfmm2d.a) and
c     hndiv2d_c_ (C translation, from build/hndiv2d.o) on a battery of
c     inputs and verifies that the (ndiv, idivflag) outputs match
c     exactly.

      program test_hndiv2d
      implicit none
     ...
     3   1.0d-12,  1.0d-15,  1.0d-16,  1.0d-20 /
      ...
      do i = 1, ntests
c        sweep ns, nt, and the if* flags so that we exercise the
c        fallback (eps below 0.5d-15) branch which depends on ns+nt.
         ns        = 100 + i
         nt        = 200 + 7*i
         ifcharge  = mod(i, 2)
         ifdipole  = mod(i+1, 2)
         ...
         call hndiv2d(eps_list(i), ns, nt, ifcharge, ifdipole,
     1        ifpgh, ifpghtarg, ndiv_f, idivflag_f)
         call hndiv2d_c(eps_list(i), ns, nt, ifcharge, ifdipole,
