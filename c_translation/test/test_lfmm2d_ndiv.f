c     test_lfmm2d_ndiv.f - differential test for the C translation of
c     src/laplace/lfmm2d_ndiv.f.
c
c     Compares the Fortran reference lfmm2d_ndiv to the C port
c     lfmm2d_ndiv_c over a sweep of the 9 combinations of
c     (ifcharge, ifdipole, ifpgh, ifpghtarg). Both sides use the same
c     random inputs (complex-valued charges and dipole strengths,
c     real dipvec); the outputs are compared element-by-element to
...
c     ndiv = 8, idivflag = 0, ifnear = 1, iper = 1.
      ...
      program test_lfmm2d_ndiv
      implicit none

      external lfmm2d_ndiv, lfmm2d_ndiv_c
      external hkrand
      real *8 hkrand
      ...
      complex *16 pottarg_f(nd, nt), pottarg_c(nd, nt)
      complex *16 gradtarg_f(nd, 2, nt), gradtarg_c(nd, 2, nt)
      complex *16 hesstarg_f(nd, 3, nt), hesstarg_c(nd, 3, nt)
...
c     ================================================================
c     Sweep test cases for lfmm2d_ndiv.
c     ================================================================
      do ic = 1, ncases
         ifcharge  = cases(1, ic)
         ifdipole  = cases(2, ic)
...
c        zero output buffers
         ...
         call lfmm2d_ndiv(nd, eps, ns, sources, ifcharge, charge,
     ...
     4        timeinfo_f, ier_f)
         ...
         call lfmm2d_ndiv_c(nd, eps, ns, sources, ifcharge, charge,
 ...
 1000 format(' [ ok ] lfmm2d_ndiv ifc=',i1,' ifd=',i1,' ifpgh=',i1,
     1     ' ifpght=',i1,'  errmax=',1pe10.3)
 1001 format(' [FAIL] lfmm2d_ndiv ifc=',i1,' ifd=',i1,' ifpgh=',i1,
     1     ' ifpght=',i1,'  errmax=',1pe10.3)
 1002 format(' [FAIL] lfmm2d_ndiv ifc=',i1,' ifd=',i1,' ifpgh=',i1,
     1     ' ifpght=',i1,'  ier mismatch ',i6,' vs ',i6)
      ...
      write(*,*) 'PASS: all lfmm2d_ndiv cases match'
