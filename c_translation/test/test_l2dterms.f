c     test_l2dterms.f - differential test for the C translation of
c     subroutine l2dterms.
c
c     Calls both l2dterms (Fortran reference, from libfmm2d.a) and
c     l2dterms_c_ (C translation, from build/l2dterms.o) on a battery
c     of eps values and verifies that (nterms, ier) match exactly.

      program test_l2dterms
      implicit none
     ...
     2   1.0d-9,   2.5d-10,  1.0d-12,  1.0d-14 /
      ...
      do i = 1, ntests
         ...
         call l2dterms(eps_list(i), nterms_f, ier_f)
         call l2dterms_c(eps_list(i), nterms_c, ier_c)
