...
// wasm: fmm2d
//
// cfmm2d_call(nd, eps, ns, sources, ifcharge, charges, ifdipole,
//             dipstr, ifpgh, nt, targ, ifpghtarg) -> complex tensor
//
// Low-level shim that calls cfmm2d_w in fmm2d.{wasm,so,dylib}.
// The matlab-facing cfmm2d.m overrides matlab/cfmm2d.m and calls this
// with column-major buffers. charges and dipstr are complex tensors;
// sources and targ are real tensors.
//
...
//   - there is NO dipvec for cfmm2d (dipoles encoded in dipstr)
//   - pot / grad / hess each have shape (nd, ns) complex (NOT (2*nd,ns)
//     and (3*nd,ns) like the real case): grad holds d/dz, hess d^2/dz^2
//
  ...
  resolve: function (argTypes, nargout) {
    ...
    return {
      ...
      apply: function (args, nargout) {
        ...
        var n_gradtarg = nd * ntuse;
        var n_hesstarg = nd * ntuse;
        ...
        if (native) {
          var fn = native.func(
            "int cfmm2d_w(int nd, double eps, int ns, double *sources," +
          ...
          var ier = fn(
            ...
            ifcharge, chargesArr, ifdipole, dipstrArr,
            ifpgh, pot, grad, hess,
            nt, targArr, ifpghtarg, pottarg, gradtarg, hesstarg
          );
          if (ier !== 0) {
            throw new RuntimeError("cfmm2d_w failed with error code " + ier);
          }
        ...
        var ier = exports.cfmm2d_w(
          nd, eps, ns, sources_ptr,
          ifcharge, charges_ptr, ifdipole, dipstr_ptr,
